...
class SupplierMedicineController extends Controller
{
    ...
    public function viewallMedFilter($supName, $searchTerm)
    {
        $med = new MedicineModel;
        $supMed = new SupplierMedicineModel;
        $man = new ManufactureModel;
    ...
    public function viewPendigFilter($supName, $searchTerm)
    {
        $med = new MedicineModel;
        $supMed = new SupplierMedicineModel;
        $labreq = new LabRequestModel;
        $man = new ManufactureModel;
        $result1 = $supMed->getPendingMedicineFilter($_SESSION['username'], $searchTerm);
        if ($result1->num_rows > 0) {
            while ($row1 = $result1->fetch_assoc()) {
                $medid = $row1["medId"];
                $medNam = $med->getName($medid);
    ...
    public function viewOtherMedFilter($supName, $searchTerm)
    {
        $supmedids = array();
        $allmedids = array();
        $otherids = array();
        ...
        $allids = $med->getallMedidFilter($searchTerm);
        while ($row2 = $allids->fetch_assoc()) {
            array_push($allmedids, $row2['id']);
        }
        $otherids = array_diff($allmedids, $supmedids);
